...
export async function loadStats(forceRefresh) {
  if (!forceRefresh) {
    try {
      ...
      if (cached) { setAllMatches(JSON.parse(cached)); renderStats(); return; }
    } catch(e) {}
  }
...
function renderMatchListPaged(matches) {
  ...
  if (totalPages > 1) {
    ...
    html += '<div class="pagination">' +
      '<button class="page-btn" data-action="setMatchPage" data-page="' + (matchPage - 1) + '" ' + (matchPage === 0 ? 'disabled' : '') + '>\u2190 Forrige</button>' +
      '<span class="page-info">' + from + '\u2013' + to + ' av ' + total + '</span>' +
      '<button class="page-btn" data-action="setMatchPage" data-page="' + (matchPage + 1) + '" ' + (matchPage >= totalPages - 1 ? 'disabled' : '') + '>Neste \u2192</button>' +
    '</div>';
  }
  return html;
}
...
function renderHomeAwaySection(matches) {
  ...
  function cardHTML(label, matchArr, colorClass) {
    ...
    return '<div class="ha-card">' +
      ...
      '<div style="display:flex;justify-content:space-between;margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">' +
        '<div style="text-align:center"><div style="font-family:Barlow Condensed,sans-serif;font-size:16px;font-weight:800;color:var(--lime)">' + s.g + '</div><div style="font-family:Barlow Condensed,sans-serif;font-size:9px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--muted)">M\u00e5l</div></div>' +
        '<div style="text-align:center"><div style="font-family:Barlow Condensed,sans-serif;font-size:16px;font-weight:800;color:var(--gold)">' + s.a + '</div><div style="font-family:Barlow Condensed,sans-serif;font-size:9px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--muted)">Assist</div></div>' +
  ...
  return '<div class="stat-row-card" style="margin-bottom:8px">' +
    ...
    '<div class="ha-grid">' +
      cardHTML('\u{1F3E0} Hjemme', homeMatches, 'home') +
...
export function renderStats() {
  ...
  if (n === 0) {
    document.getElementById('stats-content').innerHTML = '<div class="loading">Ingen kamper denne sesongen</div>';
    return;
  }
  ...
  document.getElementById('stats-content').innerHTML =
    ...
    '<div class="opponent-search-wrap">' +
      ...
      '<div class="opponent-search-field-wrap">' +
        ...
        '<input type="text" id="opponent-search-input" class="opponent-search-input" placeholder="S\u00f8k motstander..." value="' + esc(opponentSearch) + '" data-action="setOpponentSearch" />' +
...
export function renderAnalyse(matches) {
    ...
    container.innerHTML = selectorHTML + renderFormStreak(matches) +
      '<div class="chart-locked">' +
        ...
        '<div class="chart-locked-overlay">' +
          ...
          '<button class="chart-unlock-btn" data-action="showProToast">L\u00e5s opp Pro \u2b50</button>' +
        '</div>' +
      '</div>';
    return;
  }
  ...
  chartInstances['winpct'] = new Chart(document.getElementById('chart-winpct'), {
    type: 'line',
    data: { labels: labels1, datasets: [{ label: 'Seiersprosent %', data: winPctData, borderColor: CHART_COLORS.lime, backgroundColor: 'rgba(168,224,99,0.08)', borderWidth: 2, pointRadius: asc.length > 20 ? 0 : 3, pointHoverRadius: 5, pointBackgroundColor: CHART_COLORS.lime, fill: true, tension: 0.3 }] },
  ...
  chartInstances['goals'] = new Chart(document.getElementById('chart-goals'), {
    type: 'line',
    data: { labels: labels2, datasets: [{ label: 'M\u00e5l', data: goalData, borderColor: CHART_COLORS.lime, backgroundColor: 'rgba(168,224,99,0.08)', borderWidth: 2, pointRadius: asc.length > 20 ? 0 : 3, pointHoverRadius: 5, pointBackgroundColor: CHART_COLORS.lime, fill: true, tension: 0.2 }, { label: 'Assist', data: assistData, borderColor: CHART_COLORS.gold, backgroundColor: 'rgba(240,192,80,0.06)', borderWidth: 2, pointRadius: asc.length > 20 ? 0 : 3, pointHoverRadius: 5, pointBackgroundColor: CHART_COLORS.gold, fill: true, tension: 0.2 }] },
  ...
  chartInstances['tournament'] = new Chart(document.getElementById('chart-tournament'), {
    type: 'bar',
    data: { labels: tournKeys, datasets: [{ label: 'M\u00e5l', data: tournKeys.map(function(k) { return tournMap[k].g; }), backgroundColor: 'rgba(168,224,99,0.6)', borderColor: CHART_COLORS.lime, borderWidth: 1, borderRadius: 4 }, { label: 'Assist', data: tournKeys.map(function(k) { return tournMap[k].a; }), backgroundColor: 'rgba(240,192,80,0.5)', borderColor: CHART_COLORS.gold, borderWidth: 1, borderRadius: 4 }] },
